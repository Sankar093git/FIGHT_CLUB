...
function openAddModal() {
  isEditMode = false;
  editingId = null;
  document.getElementById('couponModalLabel').innerHTML =
    '<i class="bi bi-plus-circle me-2 text-primary"></i>Add New Coupon';
...
function openEditModal(
  ...
  document.getElementById('maxDiscount').value = parseInt(maxDiscount)||0;
...
function deleteCoupon(code) {
  Swal.fire({
    ...
    fetch(`/admin/coupon/${code}`, { method: 'DELETE' })
      .then((res) => res.json())
      .then((data) => {
        if (!data.success) {
        ...
        const row = document.getElementById(`couponRow${code}`);
        if (row) row.remove();
      })
      .catch(() => {
...
document.getElementById('couponForm').addEventListener('submit', function (e) {
  ...
  if (formData.discountType === 'fixed' && Number(formData.discountValue) >= Number(formData.minPurchase)) {
    Swal.fire('Error','Discount value must be lesser than the minimum purchase amount','error');
    return;
  }
...
function addNewRow(code, value, type, minPurchase,maxDiscount, limit, startDate, expiryDate) {
  try {
    const status = 'active';
    const redemptions = 0;
    const row = document.createElement('tr');
    row.id = `couponRow${code}`;
    const body = document.getElementById('body');
    const cleanStart = startDate ? new Date(startDate).toISOString() : '';
    const cleanExpiry = expiryDate ? new Date(expiryDate).toISOString() : '';
    console.log(cleanStart,cleanExpiry);
    row.innerHTML = `<td><span class="coupon-code">${code}</span></td>
              ...
              <td>
                <div class="d-flex gap-2">
                  <button class="btn btn-action btn-edit" onclick="openEditModal('${code}', ${value}, '${type}', ${minPurchase},${maxDiscount},${limit},'${cleanStart}','${cleanExpiry}')">

...
export const loadSalesReport = async (req, res) => {
    try {
        ...
        let [{ totalOrderAmount }] = await Order.aggregate([
            { $match: { createdAt: { $gte: start, $lte: end }, paymentStatus: "PAID", status: { $nin: ["Returned", "Cancelled"] } } },
            { $group: { _id: null, totalOrderAmount: { $sum: "$totalAmount" } } },
            { $project: { _id: 0 } }
        ]);
        ...
        let [{ totalOfferDiscount }] = await Order.aggregate([
            { $match: { createdAt: { $gte: start, $lte: end }, paymentStatus: "PAID", status: { $nin: ["Returned", "Cancelled"] } } },
            { $unwind: "$products" },
            { $group: { _id: null, totalOfferDiscount: { $sum: "$products.discount" } } },
            { $project: { _id: 0 } }
        ]);
        ...
        let [{ totalRefund } = { totalRefund: 0 }] = await Transactions.aggregate([
            { $match: { method: "refund", createdAt: { $gte: start, $lte: end } } },
...
export const loadChart = async (req, res) => {
    try {
        ...
        if (filter == "weekly") {
            matchFilter = { createdAt: { $gte: new Date(new Date() - 7 * 24 * 60 * 60 * 1000) } };
            groupFilter = { _id: { $dayOfWeek: "$createdAt" }, Totalamount: { $sum: "$totalAmount" }, sortDate: { $first: "$createdAt" } };
        } else if (filter == "monthly") {
            matchFilter = { createdAt: { $gte: new Date(new Date() - 30 * 24 * 60 * 60 * 1000) } };
            groupFilter = { _id: { $dateToString: { format: "%b %d", date: "$createdAt" } }, Totalamount: { $sum: "$totalAmount" }, sortDate: { $first: "$createdAt" } };
        } else if (filter == "yearly") {
            matchFilter = { createdAt: { $gte: new Date(new Date() - 365 * 24 * 60 * 60 * 1000) } };
            groupFilter = { _id: { $dateToString: { format: "%b", date: "$createdAt" } }, Totalamount: { $sum: "$totalAmount" }, sortDate: { $first: "$createdAt" } };
        } else if (filter == "daily") {
            matchFilter = { createdAt: { $gte: new Date(new Date() - 24 * 60 * 60 * 1000) } };
            groupFilter = { _id: { $dateToString: { format: "%H:00", date: "$createdAt" } }, Totalamount: { $sum: "$totalAmount" }, sortDate: { $first: "$createdAt" } };
        }
...
export const loadTopTens = async (req, res) => {
    try {
        // product rankings    
        const productRankings = await Order.aggregate([
            { $match: { paymentStatus: "PAID", status: { $nin: ["Returned", "Cancelled"] } } },
            { $unwind: "$products" },
            ...
            { $unwind: "$details" },
            { $project: { _id: 0, Name: "$details.productName", Count: 1 } }
        ]);
        ...
        const categoryRankings = await Order.aggregate([
            ...
            { $group: { _id: "$productDetails.category", Count: { $sum: "$products.quantity" } } },
        ...
        const brandRankings = await Order.aggregate([
            ...
            { $sort: { Count: -1 } },
            { $limit: 10 },
            { $project: { _id: 0, Name: "$brandDetails.brandName", Count: 1 } }
        ]);

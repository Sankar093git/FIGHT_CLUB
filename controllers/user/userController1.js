  ...
  try {
    ...
    if (!isValid) {
      return res
        ...
        .render('signUp', { message: 'One or more invalid fields present!' });
    }
...
export const login = async (req, res) => {
  try {
    ...
    if (!findUser) {
      return res.status(STATUS_CODES.BAD_REQUEST).render('login', {
    ...
    if (sessionOTP === otp) {
      ...
      if (sentMail) {
        res.status(STATUS_CODES.OK).json({
          success: true,
          message:
            'OTP verified successfully, your referral code has been sent!',
        });
      } else {
        res
          .status(STATUS_CODES.INTERNAL_SERVER_ERROR)
          .json({ success: false, message: 'Unable to send referral code!' });
      }
    } else {
      res
        .status(STATUS_CODES.OK)
  ...
  try {
    ...
    if (isExists) {
      ...
      if (sentMail) {
        res
          .status(STATUS_CODES.OK)
          .json({ success: true, message: 'OTP has been sent to your mail!' });
      } else {
        res
          .status(STATUS_CODES.INTERNAL_SERVER_ERROR)
...
export async function checkForReferal(code, email) {
  const item = await User.findOne(
    { referalCode: code, isBlocked: false },
    { email: 1 }
  );
  if (item) {
